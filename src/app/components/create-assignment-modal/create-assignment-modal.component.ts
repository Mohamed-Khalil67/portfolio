...
console.log('Assignment loaded!');`,
  ...
  get isValid(): boolean {
    return this.form.title.trim().length > 0 && this.form.html.trim().length > 0;
  }
  ...
  save(): void {
    ...
    const downloadFiles = [
      ...
      ...(this.form.css.trim() ? [{ name: 'style.css', url: '__local_css__' }] : []),
      ...(this.form.js.trim()  ? [{ name: 'script.js', url: '__local_js__'  }] : []),
    ];
    ...
    if (this.assignmentToEdit) {
      const changes: Partial<Assignment> = {
        ...
        sourceCode:  { html: this.form.html, css: this.form.css, js: this.form.js },
        downloadFiles,
      };
